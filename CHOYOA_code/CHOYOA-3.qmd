...
blank_canvas |>
  mutate(paint = gen_waves(x, y, frequency = 5))

# creating a generator using a gating method
gating_generator <- function(x, y, frequency, ...) {
  lf <- gen_perlin(x, y, frequency = frequency, ...)
  mf <- gen_simplex(y, x, frequency = frequency * 92, ...)
  hf <- gen_worley(x, y, frequency = frequency * 8, ...)
  gate <- gen_simplex(x, y, frequency = -frequency * 111, ...) 
  gate <- normalise(gate)
  paint <- lf + hf - mf*0.67 +
    (mf - 1) * (gate >= .5 & gate < .5) + 
    (hf + 9) * (gate >= .2)
...
fractal_art(ridged, gating_generator, palette = pal, seed = 9, octaves = 1)
